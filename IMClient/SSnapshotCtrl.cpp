...
void SSnapshotCtrl::OnPaint(IRenderTarget* pRT)
{
	...
	for (int i = 0; i < 8; ++i)
	{
		...
		graph.FillEllipse(&hBrush, rcDot);
	}
...
// 			m_ClickTwo = TRUE;
	...
	if (m_bDrawOperate)
	{
		m_pBitmap = new CJPBitmap(CopyCurBitmap(0, 0, m_nScreenX, m_nScreenY));
		if (m_pBitmap)
...
void SSnapshotCtrl::CalcPos()
{
	...
	// 上左 方块
	m_rcPos[(int)EcPosType::TopLeft].SetRect(rcLine.left - 4, rcLine.top - 4, rcLine.left + 4, rcLine.top + 4);
	// 上中 方块
	m_rcPos[(int)EcPosType::TopCenter].SetRect(center.x - 4, rcLine.top - 4, center.x + 4, rcLine.top + 4);
	// 上右 方块
	m_rcPos[(int)EcPosType::TopRight].SetRect(rcLine.right - 4, rcLine.top - 4, rcLine.right + 4, rcLine.top + 4);
	// 右中 方块
	m_rcPos[(int)EcPosType::RightCenter].SetRect(rcLine.right - 4, center.y - 4, rcLine.right + 4, center.y + 4);
	// 下右 方块
	m_rcPos[(int)EcPosType::BottomRight].SetRect(rcLine.right - 4, rcLine.bottom - 4, rcLine.right + 4, rcLine.bottom + 4);
	// 下中 方块
	m_rcPos[(int)EcPosType::BottomCenter].SetRect(center.x - 4, rcLine.bottom - 4, center.x + 4, rcLine.bottom + 4);
	// 下左 方块
	m_rcPos[(int)EcPosType::BottomLeft].SetRect(rcLine.left - 4, rcLine.bottom - 4, rcLine.left + 4, rcLine.bottom + 4);
	// 左中 方块
	m_rcPos[(int)EcPosType::LeftCenter].SetRect(rcLine.left - 4, center.y - 4, rcLine.left + 4, center.y + 4);
}
...
void SSnapshotCtrl::SetBmpResource(CJPBitmap* pBmp)
{
	m_pBitmap = pBmp;
...
{
	SOUI::CRect rcClient = GetClientRect();
	CJPBitmap* pBitmap = m_vecBitmap.back();

	CJPDC deskDc = CreateDC(L"DISPLAY", NULL, NULL, NULL);
	CJPDC tempDc;

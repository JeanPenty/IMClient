...
void CSnapshotDlg::OnSysCommand(UINT nID, SOUI::CPoint pt)
{
	switch (nID)
	{
	case SC_CLOSE:
...
SStringT& CSnapshotDlg::CreateNewFileName(SStringT& filename)
{
	SYSTEMTIME sys;
	GetLocalTime(&sys);
	filename.Format(_T("JeanPenty截图%4d%02d%02d%02d%02d%02d%03d"), sys.wYear, sys.wMonth, sys.wDay, sys.wHour, sys.wMinute, sys.wSecond, sys.wMilliseconds);
	return filename;
}

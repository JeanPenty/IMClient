...
class CGrpMbrTileViewAdapter : public SAdapterBase
{
public:
	struct ItemData
	{
...
public:
	CGrpMbrTileViewAdapter(IListen* pListen)
		: m_pListen(pListen)
	{

	}
	~CGrpMbrTileViewAdapter()
	{
	}
...
protected:
	...
	virtual void WINAPI getView(int position, SWindow* pItem, pugi::xml_node xmlTemplate)
	{
		if (0 == pItem->GetChildrenCount())
		{
			...
			//pItem->GetEventSet()->subscribeEvent(EventItemPanelClick::EventID, Subscriber(&CGrpMbrTileViewAdapter::OnEvtItemPanelClick, this));
		}
		...
		{
			pImg->SetAttribute(L"skin", L"skin_default_personal32");
		}
	...
	BOOL OnEvtItemPanelClick(EventArgs* e)
	{
		...
		if (pItem)
		{
			...
			if (NULL != pData)
			{
				//m_pListen->OnEmotionItemClick(pData->m_strID);
				return TRUE;
			}
		}

...
namespace SOUI
{
	...
	BOOL RichEditAudioOle::InitOleWindow(IRichEditObjHost * pHost)
	{
        ...
        std::string strAudioDuration = S_CW2A(wstrAudioDuration.c_str(), CP_UTF8);
...
// 			}
	...
	BOOL RichEditVideoOle::InitOleWindow(IRichEditObjHost* pHost)
	{
		...
		if (L"unknow" == _video_duration)
		{
			sstrVideoDuration = L"未知";
		}
		else
		{
...
// 				sstrVideoDuration.Format(L"%02d:%02d:%02d", nH, nM, nS);
	...
	void RichEditVideoOle::SetVideoPlayState(int nstate)
	{
		if (REVIDEO_STATE_DOWNFINISH == nstate)
		{
			...
			else
			{
				...
				std::string strVideoDuration;
				wstrVideoDuration = _video_duration;
				//ConvertUtf16ToUtf8(wstrVideoDuration, strVideoDuration);
				int nVideoDuration = atoi(strVideoDuration.c_str());
	...
	void RichEditVideoOle::UpdatePosition()
	{
		if (_spAdviseSink)
		{
			...
			{
				IBitmap* pFrameImg = SResLoadFromFile::LoadImage(_video_cover_path);
				int nWidth = pFrameImg->Width();
				int nHeight = pFrameImg->Height();

				SStringW sstrWidth, sstrHeight;
				sstrWidth.Format(L"%d", nWidth);
				sstrHeight.Format(L"%d", nHeight);
				wstrWidth = sstrWidth;
				wstrHeight = sstrHeight;
			}
			else
			{
			...
			//ConvertUtf16ToUtf8(wstrWidth, strWidth);
			//ConvertUtf16ToUtf8(wstrHeight, strHeight);
	...
	void RichEditLocationOle::UpdatePosition()
	{
		if (_spAdviseSink)
		{
			...
			SIZE tempSize;
			tempSize.cx = atoi(strWidth.c_str());
			tempSize.cy = atoi(strHeight.c_str()) + 150;
	...
	void RichEditLocationOle::SetLocationSize(int nWidth, int nHeight)
	{
		...
		strHeight = os.str();
		std::wstring wstrWidth, wstrHeight;
		//ConvertUtf8ToUtf16(strWidth, wstrWidth);
		//ConvertUtf8ToUtf16(strHeight, wstrHeight);
		_img_width = wstrWidth.c_str();
		_img_height = wstrHeight.c_str();
		UpdatePosition();
	}
    ...
    void RichEditReminderOle::InitAttributes()
    {
		...
		SWindow * pWnd = _oleView.FindChildByName2<SWindow>(L"parent");
		if (pWnd)
		{
			pWnd->SetAttribute(L"cursor", _cursor);
		}
    }
    ...
    BOOL RichEditReminderOle::InitOleWindow(IRichEditObjHost * pHost)
    {
        ...
        BOOL ret = RichEditOleBase::InitOleWindow(pHost);
        InitAttributes();
// 		SStatic* pAt = _oleView.FindChildByName2<SStatic>(L"LblText");
// 		if (pAt)
// 		{
// 			SUBSCRIBE(pAt, EVT_CMD, RichEditReminderOle::OnBnClickRemainderOle);
// 		}
        return ret;
    }

	LRESULT RichEditReminderOle::ProcessMessage(UINT msg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		RichEditOleBase::ProcessMessage(msg, wParam, lParam, bHandled);
		if (msg == WM_LBUTTONDOWN)
		{
...
// 	bool RichEditReminderOle::OnBnClickRemainderOle(SOUI::EventArgs* pEvt)
// 	{
// 		_pObjHost->NotifyRichObjEvent(this, CLICK_REMAINDER_OLE, 0, 0);
// 		return true;
// 	}
	...
	//
	// ------------------------------------------------------------------------------
	// impl RichEditRevokeOle
	//
	// {4CD1C11E-12E7-41b3-92BB-FA8A57B578D2}
	static const GUID IID_ReEditorOleCtrl =
	{ 0x4cd1c11e, 0x12e7, 0x41b3, { 0x92, 0xbb, 0xfa, 0x8a, 0x57, 0xb5, 0x78, 0xd2 } };

	RichEditReEditorOle::RichEditReEditorOle()
	{
		_oleGuid = IID_ReEditorOleCtrl;
		_sizeNatural.cx = 50;
		_sizeNatural.cy = 16;
		_xmlLayout = L"LAYOUT:ReEditorOleLayout";
	}
	...
	bool RichEditReEditorOle::OnLinkClicked(SOUI::EventArgs *pEvt)
	{
		int linkFlag = 0;

		SStringW linkName = pEvt->sender->GetName();
		if (linkName == _T("link_reeditor"))
		{
			_pObjHost->NotifyRichObjEvent(this, CLICK_REVOKE_REEDIT, NULL, NULL);
		}
		return true;
	}

	BOOL RichEditReEditorOle::InitOleWindow(IRichEditObjHost* pHost)
	{
		RichEditOleBase::InitOleWindow(pHost);
		SWindow * pReEditor = _oleView.FindChildByName(L"link_reeditor");
		if (pReEditor)
			SUBSCRIBE(pReEditor, EVT_CMD, RichEditReEditorOle::OnLinkClicked);

		return TRUE;
	}

	void RichEditReEditorOle::ShowLink(BOOL bShow)
	{
		_sizeNatural.cx = 0;
		_sizeNatural.cy = 0;
		UpdatePosition();
		UpdateWindowLayout(_oleView.GetWindow(_oleView.GetSwnd()));
	}
